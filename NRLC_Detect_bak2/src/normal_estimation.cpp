...
std::string normals_path="../test_output/normals/";
...
pcl::PointCloud<pcl::Normal>::Ptr fast_normal_estimation( pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, bool test,std::string name)
{
	...
	if (test)
	{
		...
		//save normals to pcd 
		pcl::io::savePCDFile(normals_path+name+"_fnormals.pcd", *normals);
		cout << "save normals to "<<normals_path<<name<<"_fnormals.pcd finish" << endl<<endl;
	}
	else
	{
		ne.compute(*normals);
