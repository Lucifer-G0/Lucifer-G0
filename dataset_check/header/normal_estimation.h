...
/*
	...
	@return: normals ptr
*/
pcl::PointCloud<pcl::Normal>::Ptr orgnized_normal_estimation(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, bool test=false,std::string name="organized_normals");

// openMP: no back ground (soft and so on, which is not useless)
pcl::PointCloud<pcl::Normal>::Ptr fast_normal_estimation(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, bool test = false, std::string name = "fast_normals");
